...
//            List<Past> listPastTenWhitoutLocked = utils.removeLockedPast(listPastTen,lockedCells);
     ...
     * @return
     */
    public String moveIA2(Board board) {
        countTurn++;
        String move = "";
        lockedCells = new ArrayList<>();
        boolean pacIsBlock = false;
        boolean availabilityCooldown = false;
        boolean oppPacIsClosed;
...
//            List<Past> listPastTenWhitoutLocked = utils.removeLockedPast(listPastTen,lockedCells);
            ...
            if (listPastTen.size() > 0) {
                ...
                if (pacIsBlock) {
                    move += this.randomMoveAvoidLastPosition(board, board.getMyPac().get(i));
                } else if (availabilityCooldown && oppPacIsClosed ) {
                    move += this.speedOrSwitch(board, board.getMyPac().get(i));
                    board.getMyPac().get(i).setTypeNotChange(false);
                } else {
                    move += " MOVE " + board.getMyPac().get(i).getPacId() + " " + bestPast.getPastX() + " "
                            + bestPast.getPastY() + "|";
                }
...
//        System.err.println("random pos:" + randomCellInt);
    ...
    public String speedOrSwitch(Board board, Pac myPac) {
        ...
        String toRock = " SWITCH " + myPac.getPacId() + " ROCK " + "Switch" + "|";
        String toScissor = " SWITCH " + myPac.getPacId() + " SCISSORS " + "Switch" + "|";
        String toPaper = " SWITCH " + myPac.getPacId() + " PAPER " + "Switch" + "|";
    ...
    public String Switch(Board board, Pac myPac) {
        boolean oppClosed = false;
        int oppId = -1;
        String oppType = "";
